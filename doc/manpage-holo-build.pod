...
installs and configures C<systemd-timesyncd> with a custom NTP server:
    ...
    [[file]]
    name     = "/etc/systemd/timesyncd.conf.d/server.conf"
    content  = """
        [Time]
    ...
    name     = "/etc/systemd/system/sysinit.target.wants/systemd-timesyncd.service"
...
=item B<--print>/B<--no-print>

By default, the resulting package will be placed in the working directory using
the naming convention for the corresponding output package format. If B<--print>
is given, the package will be printed directly to standard output instead.

=item B<--pacman>

Generate a C<pacman> package (suitable for Arch Linux and derivatives). This is
currently the only available format, but once there are multiple output formats
available, this switch can be used to override format auto-selection (which
checks the current distribution).
...
name. As a rule of thumb, the pattern C<[a-zA-Z0-9-]+> is a safe set of allowed
package names.

=item B<version> (string, required)

The package version. To ensure sanity, holo-build enforces a relatively strict
pattern, inspired by the Semantic Versioning spec. Basic version numbers must
be numbers (C<0|[1-9][0-9]*>) chained together by dots:
...
For pre-releases, a suffix of dot-separated alphanumeric labels
(C<[A-Za-z0-9-]+>) may be added, separated with a dash:
