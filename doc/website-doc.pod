...
Instead, Holo offers a configuration repository at F</holo/repo> where the
hologram can put its configuration file.

    $ pacman -Qo /holo/repo/20-webserver/etc/nginx/nginx.conf
    /holo/repo/20-webserver/etc/nginx/nginx.conf is owned by hologram-webserver 1.0.0-1
...
post-install hooks for this purpose.
    ...
    [holo]      apply /holo/repo/20-webserver/etc/nginx/nginx.conf
...
package, it always takes a copy which is placed below F</holo/backup>. This
