...
=for Comment
################################################################################
# NOTE: This document generates both the manpage and the website's doc.html,   #
#       through copious use of =begin, =end and =for.                          #
################################################################################
...
L<Perl|https://www.perl.org> to run the tests. Packages for Go and Perl are
available for all major distributions. Note that these are both build-time
dependencies; when installed, Holo is a single static binary (plus manpage) that
depends only on a UNIX kernel, plus the
...
and refuse to overwrite the manual changes.
    ...
    !! skipping target: file has been delete by user (use --force to restore)
