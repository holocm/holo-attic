...
holo B<apply> [I<-f|--force>] [I<target> ...]
...
=item B<apply> [I<-f|--force>] [I<target> ...]
...
Print a B<diff(1)> between the last provisioned version of each managed
configuration file and the actual contents of the target file.

If files are listed as arguments, diff only these files. If some of these
arguments are files not managed by Holo, a diff from C</dev/null> will be
shown.
...
(Previous releases used JSON files, which are still recognized, but deprecated.)
For example:
