...
provisioned by a plugin in the directory F</usr/share/holo/$PLUGIN_ID/> (see
C<$HOLO_RESOURCE_DIR> below). This directory must be created when the plugin is
installed, as Holo will check for its existence and refuse to use the plugin if
it is missing.

=item *

Plugins shall store persistent data in the directory
F</var/lib/holo/$PLUGIN_ID/> (see C<$HOLO_STATE_DIR> below). This directory
will be created automatically by Holo.
...
When plugins want to store calculated data from an initial scan operation, they
shall do so in the directory published by Holo in the C<$HOLO_CACHE_DIR>
environment variable. Holo will clean up this directory when it exits.

=head3 HOLO_STATE_DIR

When plugins need to store state persistently, between runs of Holo, they shall
do so in F</var/lib/holo/$PLUGIN_ID/>. This path is published by Holo in the
...
C<$HOLO_ROOT_DIR> (see below). If the state directory is missing, Holo will
create it before calling the plugin executable. However, plugins are encouraged
to create the state directory at their installation time.
...
instead of its definition, since C<$HOLO_STATE_DIR> already accounts for
C<$HOLO_ROOT_DIR> (see below). The resource directory needs to exist before
Holo is run. Plugins should create it at installation time.
