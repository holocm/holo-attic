...
The configuration file has been moved to Holo's configuration repository below
F</usr/share/holo/repo>. (Don't worry about the C<10-mywebserver> bit for now, we'll
see why it's useful in the next example.) Since C<holo-build> knows what this
is, it has automatically added C<holo> as a dependency for the package:
...
applied to the configuration from the previous configuration package.
    ...
    Working on /etc/nginx/nginx.conf
         apply /usr/share/holo/repo/10-mywebserver/etc/nginx/nginx.conf
      passthru /usr/share/holo/repo/20-alternate-port/etc/nginx/nginx.conf.holoscript

The order in which repository files are applied is defined by the directory
names below F</usr/share/holo/repo> (C<10-mywebserver> and C<20-alternate-port>), which
...
This tiny:
    ...
    -rwxr-xr-x 1 root root 2,6M  6. Nov 22:38 /usr/bin/holo-build

Holo comes in two static binaries which are trivial to deploy even during
